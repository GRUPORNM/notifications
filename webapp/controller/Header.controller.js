sap.ui.define([
    ...
    function (BaseController, JSONModel) {
        ...
        return BaseController.extend("notificationsoverview.controller.Header", {
            onInit: function () {
                var oViewModel = new JSONModel({
                    busy: false,
                    delay: 0,
                    oStandard: "qmnum,qmtxt,eqktx,aufnr,strmn,ltrmn,qmdab,notif_status,priokx,eqtypx,eartx",
                    oSmartTableView: "",
            ...
            onShortcuts: function (oEvent) {
                if (sessionStorage.getItem("shortcuts") === 'true') {
                    var that = this;
                    if (oEvent.which === 118) {
                        var oTable = sap.ui.getCore().byId("NotificationsTable").getTable(),
            ...
            onPressNotificationDetail: function (oEvent) {
                sessionStorage.setItem("goToLaunchpad", "");
                var oSource = oEvent.getSource(),
                    sPath = oSource.getBindingContext().getPath();

                this.onNavigation(sPath, "items", "/xTQAxNOTIFICATIONS_HEADER");
            },
                        ...
                        oResults.forEach(element => {
                            if (element.v_default) {
                                ...
                                if (element.variant_id != "Main") {
                                    ...
                                    // that.byId("NotificationsTable").setInitiallyVisibleFields(that.getModel("Main").getProperty("/oStandard"));
            ...
            onBuildSmartTable: function () {
                ...
                var oOldSmartTable = sap.ui.getCore().byId("NotificationsTable");
                if (oOldSmartTable) {
                    oOldSmartTable.destroy();
                }

                // Verificar se a SmartTable foi destruída corretamente
                var oDestroyedSmartTable = sap.ui.getCore().byId("NotificationsTable");
                if (!oDestroyedSmartTable) {
                    ...
                    var oSmartTable = new sap.ui.comp.smarttable.SmartTable({
                        ...
                        entitySet: "xTQAxNOTIFICATIONS_HEADER",
                        smartFilterId: "smartFilterBarGroups",
                        tableType: "ResponsiveTable",
                        header: "{i18n>notifications}",
                        showRowCount: true,
                                ...
                                if (oItems.length > 0) {
                                    ...
                                    oItems.forEach(oItem => {
                                        ...
                                        if (oItem instanceof sap.m.ColumnListItem) {
                                            ...
                                            oItem.attachPress(this.onPressNotificationDetail.bind(this));
                                            ...
                                            for (var i = 0; i < oCells.length; i++) {
                                                ...
                                                if (oCell instanceof sap.m.Text) {
                                                    ...
                                                    if (oCell.getText().indexOf("1-") !== -1 || oCell.getText().indexOf("2-") !== -1 || oCell.getText().indexOf("3-") !== -1 || oCell.getText().indexOf("4-") !== -1) {
